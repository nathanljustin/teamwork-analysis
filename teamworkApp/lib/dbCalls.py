...
def get_all_styles(test=False):
    """
    Fetch all style entries in db ordered based on entry in db

    Returns:
        List of tuples containing style entries
    """
...
def get_students_styles(student_ids, test=False):
    ...
    Returns:
        List of the calculated style scores for the student entered in the
            following order: communicator, collaborator, challenger, contributor
    """
...
def get_all_students(test=False):
    """
    Return all students' information: name, created_at, updated_at, username
    """
    with dbconnect(test) as c:
...
def get_all_student_IDs(test=False):
    """ Return all the student id numbers contined in the database """
    with dbconnect(test) as c:
...
def insert_students(students, test=False):
    """
    Insert a list of new students into database

    Args:
        students: list of (name, username) pairs for each student
    """
...
def insert_answers(values, student_ids, questions, test=False):
    """
    Insert a list of new answers into database

    Args:
        values: the values for each answer according
                to the enumeration Answer_Value
        student_ids: student's id generated by
                the database when the student was initally entered
        question: the question corresponding to each answer
    """
    length = len(values)
...
def insert_styles(styles, test=False):
    """
    Insert a list of new styles into database
